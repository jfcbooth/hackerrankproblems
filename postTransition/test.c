...
int  sum (int count,...) {
    ...
    va_start(ap, count); // initialize list
    int sum = 0;
    for(int i = 0; i < count; i++){
        sum+=va_arg(ap, int); // access next value from the argument pointer of type int
    }
...
int min(int count,...) {
    va_list ap;
    va_start(ap, count);
    int min = va_arg(ap, int); // min value is first value in list
    int cur;
